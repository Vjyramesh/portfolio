...
| Routing shell | ⚠️ Partial | `react-router` installed and wired: `src/router.tsx` defines the router (`createBrowserRouter`), `src/main.tsx` renders `<RouterProvider router={router} />` inside `<ApolloProvider>`. Two routes exist: `/` → `<App />` and `/skills/new` → `<AddSkillForm />` (verified end-to-end against the live backend + MongoDB, not just mocks); real page routes/nav for Works, Blog, Case Study, etc. not yet built |
...
| Layout (nav/footer) | ⚠️ Partial | Admin `Sidebar` component built (`src/components/Sidebar/`): `<nav>` landmark with a link per content category (Basic Info, Experience, Works, Skills, Blogs, Case Studies, White Papers, Certifications, Education, Contact Us), using `react-router`'s `NavLink` (`aria-current="page"` on the active link) and its own `sidebar` i18n namespace. Links point to `/admin/<category>` paths — those page routes are not yet registered in `src/router.tsx` (see "Open decisions"). Public site nav/footer still not started |
...
- Define page routes for each content category (Works, Blog, Case Study, White Paper, etc.) and wire them under `/admin/*` in `src/router.tsx` (the `Sidebar` component already links to these paths; also decide whether the existing `/skills/new` route should move under `/admin` for consistency).
