...
|---|---|---|---|
| Skills | ✅ `skills` query | ❌ Not started | Backend: name, category, level, proficiency, yearsOfExperience |
...
| Routing shell | ⚠️ Partial | `react-router` installed and wired: `src/router.tsx` defines the router (`createBrowserRouter`), `src/main.tsx` renders `<RouterProvider router={router} />` inside `<ApolloProvider>`. Only one route exists (`/` → `<App />`); real page routes/nav for Works, Blog, Case Study, etc. not yet built |
...
| Layout (nav/footer) | ❌ Not started | |
| `src/blocks/` scaffolding | ⚠️ Partial | Convention documented above (rule of two). Folder seeded with a `Button` block (component + colocated test + story + barrel export) as the reference pattern; other primitives (Input, Checkbox, etc.) extracted on demand. `src/components/` folder exists but empty (`.gitkeep`) |
...
- Backend `CORS_ORIGINS` needs to include the frontend's actual dev origin (Vite default `http://localhost:5173`) before live GraphQL queries will succeed — currently defaults to `http://localhost:3000`.
- Define page routes for each content category (Works, Blog, Case Study, White Paper, etc.) and a nav/layout shell — router itself is wired but only has the single root route today.
